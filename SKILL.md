...
- Run `content-ops/validator.py` through `python tests/test_content_operations.py`; unresolved provider, model, evidence, redirect, rich-text, permission, or portability decisions remain `BLOCKED` or `FAIL`.
...
- **Integration and verification:** Reuse the existing measurement event names with a locale parameter, extend the existing browser QA runner with pseudo-localization and RTL checks, transfer durable work through V2.5 Handoff, and leave production verification to Launch Operations. Run `python tests/test_localization.py`.
...
- **ENGAGE GATE LAUNCH:** Update `site-profile.json` → `launch_ops.complete = true` and `launch_ops.status = "RELEASE_READY"`; the state distinctions, authorization boundary, production verification, rollback, post-launch, and handoff rules are owned by the protocol and summarized in §5.17. This workflow performs no external side effect.
