...
- Confirm site is authorized for deployment.
...
## 6. Backward Compatibility (Pre-V1.1 through Pre-V2.9 Projects)

Website Director operates under strict backward compatibility invariants:
1. **Unmodified Legacy Pilots:** Projects created under V1 through V2.8 are frozen baselines. They are not modified, retrofitted, or invalidated by V2.9 tooling. Do not mutate frozen pilot state files merely to make them look current.
2. **Missing State Objects:** If an older project's `site-profile.json` lacks `accessibility{}`, `browser_qa{}`, `security_privacy{}`, `measurement{}`, `cro{}`, `page_experience{}`, `rive{}`, `immersive{}`, `assets{}`, `visual_prototypes{}`, `creative_intent{}`, `distinctiveness_check_complete`, `motion.gsap_required`, `design_intelligence{}`, `gauntlet{}`, or `seo{}`, tooling treats them as absent without raising schema validation exceptions.
