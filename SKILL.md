...
- Execute the **Deterministic Pre-Scan Protocol** from [IMPECCABLE-ENGINE-PROTOCOL.md](file:///c:/Users/ALPHA/Desktop/VIBE%20CODING%20PROJECTS/WEBSITE-DIRECTOR/IMPECCABLE-ENGINE-PROTOCOL.md) §3, auditing contrast math, layout transitions, easing curves, tabular figures, and browser surface styling with explicit method tagging (`DETERMINISTIC`, `HEURISTIC`, `LLM_CRITIQUE`, `VISUAL_COMPARISON`).
...
## 6. Backward Compatibility (Pre-V1.1 through Pre-V2.7 Projects)

Website Director operates under strict backward compatibility invariants:
1. **Unmodified Legacy Pilots:** Projects created under V1 through V2.6 are frozen baselines. They are not modified, retrofitted, or invalidated by V2.7 tooling. Do not mutate frozen pilot state files merely to make them look current.
2. **Missing State Objects:** If an older project's `site-profile.json` lacks `security_privacy{}`, `measurement{}`, `cro{}`, `page_experience{}`, `rive{}`, `immersive{}`, `assets{}`, `visual_prototypes{}`, `creative_intent{}`, `distinctiveness_check_complete`, `motion.gsap_required`, `design_intelligence{}`, `gauntlet{}`, or `seo{}`, tooling treats them as absent without raising schema validation exceptions.
