...
- **Asset Boundary (Asset Director Stage A Integration):** Consult `ASSET-DIRECTOR-PROTOCOL.md` §2. During prototyping, use lightweight `PROTOTYPE_ASSET_STRATEGY` with authentic client assets, licensed research imagery, or clean temporary placeholders. Do not engage in expensive bespoke 3D modeling, paid shoots, or large-scale AI generation before the owner selects a direction. If placeholders are used, record `PROTOTYPE_ASSET_LIMITATION = TRUE`—never let temporary placeholder assets disguise a strong art direction.
