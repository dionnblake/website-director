...
| `CONTENT-OPERATIONS-CMS-PROTOCOL.md` (V2.13) | Defines content types, editorial boundaries, CMS decision, portability, migration, and slug continuity | **Consumed, not duplicated.** Launch verifies that the known release uses the selected content source/provider, routes, required fields, and redirects. Content model authority remains Phase 6.25; long-term client CMS operations remain V2.5 handoff. |
