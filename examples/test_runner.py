...
#
# What this harness proves: the V2.0-V2.15 protocol documents, master templates,
...
def run():
    print('=== WEBSITE DIRECTOR V2.0-V2.15 PROTOCOL, TEMPLATE & PILOT INVARIANT HARNESS ===\n')
    ...
    for tmpl in ['templates/asset-intent-brief.md', 'templates/photography-shot-list.md', 'templates/asset-manifest.json', 'templates/asset-provenance.md', 'templates/evidence-ledger.md', 'templates/evidence-ledger.json', 'templates/immersive-implementation-brief.md', 'templates/rive-implementation-brief.md', 'templates/page-experience-brief.md', 'templates/analytics-measurement-plan.md', 'templates/experiment-brief.md', 'templates/analytics-event-manifest.json']:
    ...
    for obj in ('assets', 'immersive', 'rive', 'page_experience', 'measurement',
                'security_privacy', 'provenance', 'content_ops', 'localization', 'application', 'launch_ops'):
    ...
    assert 'provenance' in sp and sp['provenance'].get('complete') is False, \
        'provenance{} must ship neutral (not complete)'
    assert sp['provenance'].get('ledger_ref') == 'evidence-ledger.json', \
        'provenance{} must point to the canonical ledger template'
    loc = sp['localization']
    assert loc.get('required') is False and loc.get('complete') is False, \
        'localization{} must ship neutral (not required, not complete)'
    assert loc.get('implementation_verified') is False and loc.get('production_verified') is False, \
        'localization verification flags must default false'
    assert not any(str(key).lower().endswith('_locked') for key in loc), \
        'localization{} must not create an owner lock'

    app = sp['application']
    assert app.get('required') is False and app.get('complete') is False, \
        'application{} must ship neutral and conditional'
    assert app.get('status') == 'NOT_REQUIRED', \
        'application{} must default to NOT_REQUIRED'
    assert app.get('implementation_verified') is False and app.get('production_verified') is False, \
        'application verification flags must default false'
    assert not any(str(key).lower().endswith('_locked') for key in app), \
        'application{} must not create an owner lock'
    for parallel in ('auth', 'auth_complete', 'commerce', 'commerce_complete',
                     'payments', 'payments_complete', 'application_locked'):
        assert parallel not in sp, f'parallel application state {parallel!r} must be absent'
