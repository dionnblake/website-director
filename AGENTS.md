...
- Current owner decision for clean-room run `asn-real-proof-20260908-565be6d8`: `OWNER_CONCEPT_SELECTION = CONCEPT_B`. B is the sole visual direction; A is rejected; C is a craft-quality benchmark only, never an art-direction or asset source. Preserve B's oversized editorial typography, neutral paper, structural navy, controlled yellow, asymmetry and physical planning-document language. No hybrid, hardware/dial/control-panel import, or reuse of previous ASN visual systems. One B-only desktop/mobile homepage pass and at most one bounded refinement are authorized after evaluator repair verification; homepage acceptance remains PENDING. The owning continuation contract is linked below. Earlier rejected runs and frozen projects remain unchanged; no merge, deployment or production changes are authorized.

## Child DOX Index

- [Concept B homepage continuation](work/v2-11-clean-room-creative-mode/.clean-room-runs/asn-concept-b-homepage-20260909/AGENTS.md): Owner-selected B direction, immutable concept evidence, evaluator recheck, existing owner-selection authority, bounded homepage design, craft-only C benchmark and pending homepage acceptance.

- [outputs/reference-fidelity-benchmark/](outputs/reference-fidelity-benchmark/AGENTS.md): Three preserved desktop reference experiments plus one owner-authorized Taste-only hero ablation, captured reference evidence, execution receipts, and owner visual comparisons. No self-grading or automatic diagnosis.
...
- [framework_validation/](framework_validation): Standard-library deterministic framework self-validation, including the bounded cinematic/inspiration, rendered-visual evidence, and visual-first taste architecture helpers. It does not call providers, mutate frozen projects, or replace Website Director authorities.
- [candidates/](candidates): Isolated, local-only Website Director evaluation candidates. Candidate builds may contain their own specifications, production surfaces, research records, and evidence, but do not own framework architecture, frozen certification pilots, deployment, publishing, or production verification.
- [Website Director Test Sites/](<Website Director Test Sites>): Local-only copies of recent launchable Website Director site surfaces for visual review. The collection is not a canonical source and does not authorize deployment or publishing.
...
The bounded visual-first overlay is documented in
`VISUAL-FIRST-ARCHITECTURE.md`. Its project-local `taste_contract`, dynamic
concept routing, feasibility check, owner selection, hot-path compiler, and
pairwise visual gate extend existing authorities without adding a sixth lock,
provider dependency, or replacement implementation engine.
